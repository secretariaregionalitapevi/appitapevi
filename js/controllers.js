 ...
 *
 */
function MainCtrl($http, AuthService, $state) {
...
function ValidationService() {
     ...
     */
    self.validatePasswordStrength = function (password) {
        ...
        if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
            return { valid: true, message: 'Recomendado: adicionar caracteres especiais para maior segurança', warning: true };
        }
...
function registerCtrl($scope, AuthService, ValidationService, $state, $injector) {
    ...
    $scope.validateForm = function () {
        ...
        // Valida nome
        var nameValidation = ValidationService.validateName($scope.user.name);
        if (!nameValidation.valid) {
            $scope.errors.name = nameValidation.message;
            isValid = false;
        }

        // Valida email
        if (!ValidationService.validateEmail($scope.user.email)) {
            $scope.errors.email = 'Email inválido';
            isValid = false;
        }

        // Valida senha
        var passwordValidation = ValidationService.validatePasswordStrength($scope.user.password);
        if (!passwordValidation.valid) {
            $scope.errors.password = passwordValidation.message;
            isValid = false;
        } else if (passwordValidation.warning) {
            $scope.errors.passwordWarning = passwordValidation.message;
        }
    ...
    $scope.$watch('user.name', function () {
        if ($scope.user.name && $scope.errors.name) {
            var validation = ValidationService.validateName($scope.user.name);
            if (validation.valid) {
    ...
    $scope.$watch('user.email', function () {
        if ($scope.user.email && $scope.errors.email) {
            if (ValidationService.validateEmail($scope.user.email)) {
    ...
    $scope.$watch('user.password', function () {
        if ($scope.user.password && $scope.errors.password) {
            var validation = ValidationService.validatePasswordStrength($scope.user.password);
            if (validation.valid) {
...
function loginCtrl($scope, AuthService, ValidationService, $state, $injector, SweetAlert, $timeout) {
    ...
    $scope.validateForm = function () {
        ...
        if ($scope.credentials.email && $scope.credentials.email.indexOf('@') !== -1) {
            ...
            $scope.errors.email = 'Usuário ou Email inválido';
            isValid = false;
        }

        if (!$scope.credentials.password || $scope.credentials.password.length < 1) {
    ...
    $scope.submitLogin = function () {
        ...
        var loginData = {
            email: $scope.credentials.email.trim().toLowerCase(),
